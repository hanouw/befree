...
const SelectionButtons = ({ items, selectedItems, toggleFunction }) =>
  items.map((item) => (
    <button key={item} className={selectedItems.includes(item) ? "selected" : ""} onClick={() => toggleFunction(item)}>
      {item}
    </button>
  ));
...
const TripPlanAddComponent = (region) => {
  ...
  const handleSubCategoryClick = (subCategory) => {
    toggleSelection(setSelectedSubCategories, selectedSubCategories, subCategory);
  };
  ...
  const handleRemoveSelected = (item) => {
    if (selectedProvinces.includes(item)) {
      ...
      setSelectedSubCategories((prevSelected) => prevSelected.filter((selectedItem) => selectedItem !== item));
  ...
  console.log(
    // 전부 리스트 형태
    selectedProvinces, // 서울특별시 X
    selectedCities, // 서울특별시 강남구 O
    selectedCategories, // 자연 X
    selectedSubCategories, // 한식', '쇼핑1', '숙박1', '레포츠2', '추천코스3', '인문3', '자연3' O
    selectedAccessibilityTypes, // 청각장애인 O
    selectedFacilities, // 기저귀 교환대 있음 O
    cityOptions, // 전체 강남구, 강북구, 강동구
    currentProvince // 서울특별시 ( 도/광역시 )
  );

  const searchAllWithKeyWord = () => {
    placeKeywordData(1, "O", null, null, region, null, null, null);
    ...
    <div className="container font-['Pretendard'] text-center">
            ...
            <div className="sub-options flex flex-wrap justify-center">
              {selectedCategories.flatMap((category) =>
                categories[category].map((subCategory) => (
                  <button
                    key={subCategory}
                    className={`m-1 ${selectedSubCategories.includes(subCategory) ? "selected" : ""}`}
      ...
      <div className="page-footer">
        <div className="selected-area flex flex-wrap justify-center">
          {[...selectedCities, ...selectedSubCategories, ...selectedAccessibilityTypes, ...selectedFacilities].map(
            (item) => (
        ...
        <div className="buttons flex justify-center mt-4">
          ...
          <button onClick={searchAllWithKeyWord}>검색하기</button>
        </div>
      </div>
    </div>
  );
